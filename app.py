...
def is_faq_heading(heading):
    return bool(re.search(r"<h2[^>]*>.*?faq.*?</h2>", heading, re.I))
...
def add_dynamic_blocks(blocks, sections_stream, templates):
    i = 0
    for section in sections_stream:
        if "button_block" in section:
            blocks.append(("__button_block__", [section]))
        else:
            blocks.append((templates[i % len(templates)], [section]))
            i += 1
    return blocks


# ---------- Builders ----------
def build_dynamic_template(sections, intro_template, middle_templates, use_map, h1, token_map):
    first_idx = next(i for i, s in enumerate(sections) if "heading" in s)

    working = sections[first_idx:]
    ...
    outro_file = get_outro_filename(blocks[-1][0], use_map)
    blocks.append((outro_file, [outro]))

    return render_blocks(blocks, h1, token_map)


def build_dealer_near_template(sections, use_map, h1, token_map):

    pre = []
    while sections and "button_block" in sections[0]:
        pre.append(sections.pop(0))
    ...
    if pre:
        pre_blocks = [("__button_block__", [b]) for b in pre]
        pre_html, _ = render_blocks(pre_blocks, h1, token_map)
...
def index():
    ...
    if request.method == "POST":
        ...
        first = find_first_heading_section(sections)
        if not first:
            return redirect(url_for("error_game"))

        h1 = strip_all_html(clean_heading(first["heading"]))

        if dealer:
            output, err = build_dealer_near_template(sections, map_toggle, h1, token_map)
        elif srp:
            output, err = build_dynamic_template(
                sections,
                "Intro.txt",
        ...
        filename = re.sub(r"[^a-zA-Z0-9]+", "_", h1).lower()[:50] + ".txt"
