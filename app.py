...
# ---------- BUTTON SYSTEM ----------
# We use a sentinel token so extract_sections can treat it as a standalone "section"
...
def replace_button_blocks(html):
    ...
    def make_token(html_snippet):
        ...
        # Put newlines around it so the regex scanner can reliably see it as its own chunk
...
def apply_button_tokens(text, token_map):
    ...
    # Replace sentinel tokens with HTML
    text = re.sub(BTN_TOKEN_PATTERN, repl, text, flags=re.I | re.S)

    # Backward compatibility: if old <p data-upi="TOKEN"></p> ever exists, still replace it
...
def extract_sections(html_content):
    ...
    for match in matches:
        ...
        # Button token becomes its own section in the stream
        token_match = re.match(BTN_TOKEN_PATTERN, tag, flags=re.I | re.S)
        if token_match:
            # flush current section even if it has only heading (intro might have heading + some paragraphs)
...
def add_dynamic_blocks_for_sections(blocks, sections_stream, middle_templates):
    """
    Appends blocks in the same order as sections_stream.
    Button blocks become their own "__button_block__" block.
    Only real content sections increment the alternating template index.
    """
    tmpl_i = 0
...
def build_dynamic_template(sections, intro_template, middle_templates, use_map_outro=False, global_h1_plain=None, token_map=None):
    ...
    # Insert middle content and button blocks in order
...
def build_mslp_template(sections, use_map_outro=False, global_h1_plain=None, token_map=None):
    # Same idea, but hoverbox sections need groups of 3 real content sections.
    if len(sections) < 4:
...
def build_hubpage_template(sections, use_map_outro=False, global_h1_plain=None, token_map=None):
    ...
    while i < len(content_sections):
        ...
        if toggle:
            # need 2 real content sections for side-by-side
...
def index():
    ...
    if request.method == "POST":
        ...
        # Must have a real first heading section
